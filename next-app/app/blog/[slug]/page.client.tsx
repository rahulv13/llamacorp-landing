...
import CalloutNode from '@/components/sections/admin/editor-nodes/CalloutNode';
...
export default function BlogArticleClient({ initialPost, initialRelatedPosts }) {
  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4">
        ...
        <Link href="/blog" className="px-6 py-3 bg-[#111] text-white rounded-full font-medium">
          Back to Blog
        </Link>
      </div>
    );
  ...
  return (
      ...
      <main className="pt-32 pb-20 min-h-screen bg-white">
        ...
        <header className="max-w-4xl mx-auto px-4 md:px-8 mb-12">
          ...
          <div className="flex items-center justify-between border-y border-black/5 py-6">
            <div className="flex items-center gap-4">
              <Image src={post.author?.avatar || 'https://ui-avatars.com/api/?name=' + (post.author?.name || 'Author')} alt={post.author?.name || 'Author'} width={48} height={48} className="rounded-full object-cover" />
              <div>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 mb-16">
          <div className="w-full aspect-[21/9] rounded-[32px] overflow-hidden bg-black/5 relative">
              ...
              {relatedPosts.map((relatedPost) => (
                <Link href={`/blog/${relatedPost.slug}`} key={relatedPost._id} className="group block bg-white rounded-[24px] border border-black/5 overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300">
                  <div className="relative h-48 overflow-hidden">
                    <Image src={relatedPost.coverImage && relatedPost.coverImage !== 'no-photo.jpg' ? relatedPost.coverImage : 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80'} alt={relatedPost.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
