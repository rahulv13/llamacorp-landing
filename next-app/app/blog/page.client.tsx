...
export default function BlogClient({ initialBlogs = [], initialCategories = ['All'] }) {
                    ...
                    <div className="md:w-2/5 p-8 md:p-12 flex flex-col justify-center bg-white relative z-10">
                      ...
                      <div className="flex items-center justify-between mt-auto">
                        <div className="flex items-center gap-3">
                          <Image src={featuredPost.author?.avatar || 'https://ui-avatars.com/api/?name=' + (featuredPost.author?.name || 'Author')} alt={featuredPost.author?.name || 'Author'} width={40} height={40} className="rounded-full object-cover" />
                    ...
                    >
                      <Link href={`/blog/${post.slug}`} className="group block h-full bg-white rounded-[24px] border border-black/5 overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300">
                        <div className="relative h-48 overflow-hidden">
                          <Image src={post.coverImage && post.coverImage !== 'no-photo.jpg' ? post.coverImage : 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80'} alt={post.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <div className="flex flex-col gap-6">
                    {popularPosts.length > 0 ? popularPosts.map((post, index) => (
                      <Link href={`/blog/${post.slug}`} key={post._id} className="group flex gap-4 items-center">
                        <div className="w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-black/5 relative">
                          <Image src={post.coverImage && post.coverImage !== 'no-photo.jpg' ? post.coverImage : 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80'} alt={post.title} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
                        </div>
                        <div>
                ...
                <div className="flex flex-col">
                  {latestPosts.map((post) => (
                    <Link href={`/blog/${post.slug}`} key={`list-${post._id}`} className="group py-6 border-b border-black/5 flex flex-col md:flex-row gap-6 md:items-center hover:bg-black/[0.02] transition-colors px-4 -mx-4 rounded-xl">
                      <div className="w-full md:w-48 h-32 md:h-24 shrink-0 rounded-xl overflow-hidden relative">
                        <Image src={post.coverImage && post.coverImage !== 'no-photo.jpg' ? post.coverImage : 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80'} alt={post.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                      </div>
