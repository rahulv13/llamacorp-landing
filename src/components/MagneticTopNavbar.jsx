...
export default function MagneticTopNavbar() {
  ...
  const rightActions = [
    { type: 'icon', label: 'WhatsApp', bg: 'bg-transparent', index: 4, icon: (
    ...
    { type: 'icon', label: 'Telegram', bg: 'bg-transparent', index: 5, icon: (
    ...
    { type: 'button', label: "Let's Work", index: 6 }
  ];
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-4 border-b border-black/5 pb-4">
              {centerLinks.map((item) => (
                <a key={item.label} href="#" className="text-[16px] font-medium text-[#111]">{item.label}</a>
              ))}
            </div>
            <div className="flex gap-4 pt-2">
              {rightActions.filter(item => item.type === 'icon').map((item) => (
                <a key={item.label} href="#" className={`flex items-center justify-center w-10 h-10 rounded-full ${item.bg}`}>
                  {item.icon}
                </a>
              ))}
            </div>
