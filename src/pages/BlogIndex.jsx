...
import { getFeaturedPost, getLatestPosts, getPopularPosts, getAllCategories } from '../data/blogData';
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto min-h-screen">
                ...
                <div className="md:w-2/5 p-8 md:p-12 flex flex-col justify-center bg-white relative z-10">
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-[#111] text-sm">{featuredPost.author}</span>
                    </div>
                  ...
                  <Link to={`/blog/${post.slug}`} className="group block h-full bg-white rounded-[24px] border border-black/5 overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between text-xs text-[#777] pt-4 border-t border-black/5">
                        <span className="font-medium text-[#111]">{post.author}</span>
            ...
            <div className="sticky top-32">
              ...
              <div className="flex flex-col gap-6">
                {popularPosts.map((post, index) => (
                  <Link to={`/blog/${post.slug}`} key={post.id} className="group flex gap-4 items-center">
                    <div className="w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-black/5">
                      <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                    </div>
                    <div>
                      <div className="text-[10px] font-bold uppercase tracking-wider text-blue-600 mb-1">{post.category}</div>
        ...
        <section className="mb-24">
          ...
          <div className="flex flex-col">
            {latestPosts.map((post, index) => (
              <Link to={`/blog/${post.slug}`} key={`list-${post.id}`} className="group py-6 border-b border-black/5 flex flex-col md:flex-row gap-6 md:items-center hover:bg-black/[0.02] transition-colors px-4 -mx-4 rounded-xl">
                <div className="w-full md:w-48 h-32 md:h-24 shrink-0 rounded-xl overflow-hidden">
                  <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                </div>
                <div className="flex-1">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-[#777] mb-2">{post.category} • {post.date}</div>
                  <h4 className="text-xl font-bold text-[#111] mb-2 group-hover:text-blue-600 transition-colors">{post.title}</h4>
                  <p className="text-[#555] text-sm line-clamp-1">{post.excerpt}</p>
                </div>
